...
if __name__ == "__main__":
    ...
    # ----------------------------------------------------------------------------------------------------------------------------------
    data = np.load("{}/baseline_weights_{}_{}_{}.npz".format(directory, n_profiling, dataset_name, model_name), allow_pickle=True)
    ...
    target_byte = key_byte
    correct_key = bytearray.fromhex(key)[key_byte]
    ...
    # ----------------------------------------------------------------------------------------------------------------------------------
    # 2.2. Train baseline model for current key byte
    # ----------------------------------------------------------------------------------------------------------------------------------
    if model_name == "mlp4":
        model = mlp4(classes, ns)
    elif model_name == "mlp6":
    ...
    guessing_entropy_callback = GuessingEntropy(key_rank_runs, key, correct_key, l_model, target_byte, X_attack, attack_data,
    ...
    model.fit(
        ...
        callbacks=[guessing_entropy_callback])

    ge_baseline, sr_baseline = compute_ge(key_rank_runs, model, key, correct_key, l_model, target_byte, X_attack, attack_data,
                                          key_rank_report_interval, key_rank_number_of_traces)

    ge_baseline_epochs = guessing_entropy_callback.get_ge_epochs()
    sr_baseline_epochs = guessing_entropy_callback.get_sr_epochs()
    ...
    # ----------------------------------------------------------------------------------------------------------------------------------
    # 2.3. Create Pruned Model from Baseline Model
    # ----------------------------------------------------------------------------------------------------------------------------------
    ...
    # ----------------------------------------------------------------------------------------------------------------------------------
    # 2.4. Re-initialize and train Pruned Model with initial weights from Baseline Model (Lottery Ticket)
    # ----------------------------------------------------------------------------------------------------------------------------------
    ...
    model.fit(
        ...
        shuffle=True,
        validation_data=(X_attack, Y_attack),
        callbacks=[PrunerCallback(pruner), guessing_entropy_callback])
    ...
    ge_lottery_ticket_epochs = guessing_entropy_callback.get_ge_epochs()
    sr_lottery_ticket_epochs = guessing_entropy_callback.get_sr_epochs()

    # ----------------------------------------------------------------------------------------------------------------------------------
    # 2.5. Initialize and train Pruned Model with get random uniform weights
    # ----------------------------------------------------------------------------------------------------------------------------------
    if model_name == "mlp4":
        model = mlp4(classes, ns)
    elif model_name == "mlp6":
    ...
    ge_pruning_epochs = guessing_entropy_callback.get_ge_epochs()
    sr_pruning_epochs = guessing_entropy_callback.get_sr_epochs()
    ...
    np.savez(
        "{}/lth_pruning_sparsity_{}_traces_{}_{}_{}_{}epochs_all_key_byte_{}_{}.npz".format(directory,
                                                                                            sparsity_level,
                                                                                            n_profiling,
                                                                                            dataset_name,
                                                                                            model_name,
        ...
        ge_baseline_epochs=ge_baseline_epochs,
        ge_pruning_epochs=ge_pruning_epochs,
        ge_lottery_ticket_epochs=ge_lottery_ticket_epochs,
        sr_baseline_epochs=sr_baseline_epochs,
        sr_pruning_epochs=sr_pruning_epochs,
        sr_lottery_ticket_epochs=sr_lottery_ticket_epochs
    )
