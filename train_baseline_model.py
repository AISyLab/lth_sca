...
if __name__ == "__main__":
    ...
    model.fit(
        ...
        callbacks=[callback_weights, guessing_entropy_callback])

    print(guessing_entropy_callback.get_ge_epochs())

    best_epoch = np.argmin(guessing_entropy_callback.get_ge_epochs())
    print("Best Epoch Baseline Model: {} (GE = {})".format(best_epoch, min(guessing_entropy_callback.get_ge_epochs())))
    trained_weights = callback_weights.get_weights()[int(best_epoch)]

    np.savez("{}/baseline_weights_{}_{}_{}.npz".format(directory, n_profiling, dataset_name, model_name),
             initial_weights=initial_weights, trained_weights=trained_weights)
